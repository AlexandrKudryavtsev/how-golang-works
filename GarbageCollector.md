...
## Триколорная раскраска и барьеры записи

Go GC использует вариант параллельного (concurrent) mark-and-sweep, что означает, что сборка мусора происходит параллельно с выполнением основной программы (хотя и с некоторой приостановкой). Для этого используется ***триколорная раскраска объектов***.

Во время фазы пометки, каждому объекту присваивается один из трех цветов:

- Белый: Объект еще не был достигнут GC. В конце фазы пометки все белые объекты считаются мусором.
- Серый: Объект был достигнут, но его потомки еще не были просканированы. Серые объекты находятся в "рабочем списке" GC.
- Черный: Объект был достигнут, и все его потомки были просканированы. Черный объект считается "живым".

Изначально все объекты белые. GC начинает с корней, окрашивая их в серый цвет. Затем, GC берет серый объект, сканирует его потомков, окрашивая их в серый цвет, а сам объект красит в черный. Этот процесс повторяется до тех пор, пока не останется серых объектов.

Проблема параллельной сборки мусора заключается в том, что программа может изменять граф объектов во время работы GC. Это может привести к ситуации, когда GC пропустит "живой" объект. Чтобы предотвратить это, используются барьеры записи.

***Барьеры записи (write barriers)*** - это код, который выполняется при каждом присваивании указателя. Они гарантируют, что граф объектов останется согласованным во время работы GC. В Go используются два основных типа барьеров записи:
-  ***Барьер записи Дейкстры (удаление указателей)***: если черный объект получает указатель на белый объект, то белый объект окрашивается в серый цвет. Это предотвращает ситуацию, когда черный объект "скрывает" белый объект от GC. Этот барьер удаляет возможность появления "висячих" ссылок.
- ***Барьер записи Юаса (добавление указателей)***: когда происходит изменение указателя, старый указатель (который мог указывать на белый объект) добавляется в специальный буфер. Позже GC сканирует этот буфер, чтобы убедиться, что ни один "живой" объект не был пропущен.

Go использует гибридную модель барьера записи. Барьеры записи добавляют небольшую нагрузку на программу, но они необходимы для обеспечения корректной и параллельной сборки мусора.
