...
При вызове метода `ValueOf` берется значение `_type` из `eface`:
...
Здесь создается `unsafe.Pointer`, который может не учитывается GC. Это означает, что результат `ValueOf` тесно связан с передаваемым значением. Такая связь приведет к утечке памяти, в случае, если результат `ValueOf` не будет удален, например, он будет записан в глобальную память:
